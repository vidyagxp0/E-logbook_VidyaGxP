...
function Sidebar() {
    ...
    return (
        <aside className={`fixed z-50 bg-gray-800 text-white h-full transition-transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
            <div className="inner-block p-4">
                ...
                <Each of={asideLinks} render={(item) =>
                    <div className="sidebar-link mb-2" key={item.id}>
                        {item.hasChild ? 
                            <div className="link-head flex justify-between items-center cursor-pointer p-2" onClick={() => toggleItem(item.id)}>
                            ...
                            <Link to={item.link} className="link-head flex items-center p-2">
                                {item.icon}
                                <div className="title ml-2">{item.title}</div>
                            </Link>
                        }
                            ...
                            <div className="sidebar-subList pl-6">
                                {item.child ? 
                                    <Each of={item.child} render={(child) => 
                                        <NavLink to={child.link} className="sidebar-subLink block p-2 text-sm">
