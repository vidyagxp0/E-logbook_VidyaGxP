...
function Sidebar() {
  ...
  return (
    <aside className={`sidebar ${isCollapsed ? "collapsed" : ""}`}>
      <div className="toggle-button pt-4 pl-5" onClick={() => setIsCollapsed(!isCollapsed)}>
        <FaBars />
      </div>
      {/* Conditionally render content based on isCollapsed */}
      {!isCollapsed && (
        <div className="inner-block">
          {asideLinks && Array.isArray(asideLinks) && (
            <Each
              of={asideLinks}
              render={(item) => (
                    ...
                    <div className="sidebar-subList">
                      {item.child ? (
                        <Each
                          of={item.child}
                          render={(child) => (
                            <NavLink to={child.link} className="sidebar-subLink" key={child.id}>
