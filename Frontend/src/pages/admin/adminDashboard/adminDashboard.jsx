...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const url = "http://192.168.1.29:1000/user/get-all-users"; // Assuming the endpoint is corrected to "/user/get-all-users"
    axios
      .get(url, {
        headers: {
  ...
  const handleDelete = () => {
    const config = {
      method: "delete",
      url: `http://192.168.1.29:1000/user/delete-user/${selectedUser.user_id}`,
      headers: {
  ...
  const openPermissionsModal = (user) => {
    const token = localStorage.getItem("admin-token");
    axios
      .get(`http://192.168.1.29:1000/user/get-user-permissions/${user.user_id}`, {
        headers: {
