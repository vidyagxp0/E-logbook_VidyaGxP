...
const SdsPageEffective = () => {
  ...
  const handlePopupSubmit = (credentials) => {
    ...
    if (popupAction === "sendFromOpenToReview") {
      ...
      axios
        .put("http://localhost:1000/sds-page/approve-elog", data, config)
        .then(() => {
      ...
      const requestOptions = {
        ...
        url: "http://localhost:1000/sds-page/update",
      };
  ...
  const addRow = () => {
    if (
      ...
      setEditData((prevState) => ({
        ...prevState,
        sdsPageRecords: [...prevState?.sdsPageRecords, newRow],
      }));
  ...
  const deleteRow = (index) => {
    if (
      ...
      const updatedGridData = [...editData.sdsPageRecords];
      ...
      setEditData((prevState) => ({
        ...prevState,
        sdsPageRecords: updatedGridData,
      }));
    }
  };
  ...
  const filteredGridData = useMemo(() => {
    const records = editData?.sdsPageRecords || [];
  ...
  const generateEmptyReport = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:1000/sds-page/blank-report/${formId}`,
        {
  ...
  const allRecordDates = editData?.sdsPageRecords?.map((r) => new Date(r.date));
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:1000/sds-page/effective-chat-pdf/${formId}`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("user-token")}`,
  ...
  const handleDeleteFile = async (index) => {
    const record = editData.sdsPageRecords[index];
...
export default SdsPageEffective;
