...
const VacuumOvenOpEffective = () => {
  ...
  const handlePopupSubmit = (credentials) => {
    const cleanedData = editData?.voCalibRecords.filter((record) => {
 ...
 const addRow = () => {
  ...
  const deleteRow = (index) => {
    if (
      ...
      const updatedGridData = [...editData.voCalibRecords];
      ...
      setEditData((prevState) => ({
        ...prevState,
        voCalibRecords: updatedGridData,
      }));
    }
  };
  ...
  const generateEmptyReport = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:1000/vo-cal/blank-report/${formId}`,
        {
  ...
  const allRecordDates = editData?.voCalibRecords?.map((r) => new Date(r.date));
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:1000/vo-cal/effective-chat-pdf/${formId}`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("user-token")}`,
  ...
  const handleDeleteFile = async (index) => {
    const record = editData.voCalibRecords[index];
                    ...
                    <table>
                      <thead>
                        <tr>
                          ...
                          <th className="!text-nowrap">Batch No./Reg No.</th>
...
export default VacuumOvenOpEffective;
