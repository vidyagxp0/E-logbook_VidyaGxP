...
const UvWITransilluminatorEffective = () => {
  ...
  const handlePopupSubmit = (credentials) => {
    const cleanedData = editData?.uvWhiteLightRecords.filter((record) => {
    ...
    const updatedEditData = {
      ...editData,
      uvWhiteLightRecords: cleanedData,
    };
    ...
    if (popupAction === "sendFromOpenToReview") {
      ...
      axios
        .put("http://localhost:1000/uv-wl-transi/approve-elog", data, config)
        .then(() => {
      ...
      const requestOptions = {
        ...
        url: "http://localhost:1000/uv-wl-transi/update",
      };
  ...
  const addRow = () => {
    if (
      ...
      setEditData((prevState) => ({
        ...prevState,
        uvWhiteLightRecords: [...prevState?.uvWhiteLightRecords, newRow],
      }));
  ...
  const deleteRow = (index) => {
    if (
      ...
      const updatedGridData = [...editData.uvWhiteLightRecords];
      ...
      setEditData((prevState) => ({
        ...prevState,
        uvWhiteLightRecords: updatedGridData,
      }));
    }
  };
  ...
  const filteredGridData = useMemo(() => {
    const records = editData?.uvWhiteLightRecords || [];
  ...
  const generateEmptyReport = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:1000/uv-wl-transi/blank-report/${formId}`,
        {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:1000/uv-wl-transi/effective-chat-pdf/${formId}`,
        payload,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("user-token")}`,
  ...
  const handleDeleteFile = async (index) => {
    const record = editData.uvWhiteLightRecords[index];
            ...
            <div className="details-form-data">
              <div className="sub-head-2 p-4 bg-white rounded-md shadow-md flex flex-col sm:flex-row justify-between items-center">
                <span className="text-lg font-semibold text-white mb-4 sm:mb-0">
                  Operation of UV & White-Light Transilluminator
                </span>
                    ...
                    onClick={() =>
                      navigate("/effective-audit-trail", {
                        state: {
                          ...
                          process: "UV/WL Transilluminator",
                        },
                      })
                    }
                  >
...
export default UvWITransilluminatorEffective;
