...
export default function EquipmentUsagePanel() {
    ...
    if (popupAction === "sendFromOpenToReview") {
      ...
      if (
        editData?.EquipmentRecords?.some(
          (record) => record.equipment_record === "" || record.remarks === ""
        )
      ) {
  ...
  const addRow = () => {
    if (
      ...
      setEditData((prevState) => ({
        ...
        Equipment_Record: [...prevState.Equipment_Record, newRow],
      }));
    }
  };
  ...
  const deleteRow = (index) => {
    if (
      ...
      const updatedGridData = [...editData.Equipment_Record];
      updatedGridData.splice(index, 1);
      setEditData((prevState) => ({
        ...prevState,
        Equipment_Record: updatedGridData,
      }));
    }
  };
  ...
  const generateReport = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:1000/equipment-usage/chat-pdf/${formId}`,
        {
              ...
              {isSelectedDetails === true ? (
                <>
                  ...
                  <div className="group-input">
                      ...
                      <label className="color-label mb-4">Equipment ID. </label>
                      <div>
                        <input
                          type="text"
                          name="equipment_id"
                  ...
                  <table>
                    ...
                    <tbody>
                      {/* {editData?.Equipment_Record.map((item, index) => (
                        <tr key={index}>
                          <td>{index + 1}</td>
                          ...
                          <td>
                            <input
                              type="number"
                              value={item.equipment_record}
                              className={`${
                                item.equipment_record < 23
                                  ? "limit"
                                  : item.equipment_record > 27
                                  ? "limit"
                                  : ""
                              }`}
                              onChange={(e) => {
                                const newData = [...editData.Equipment_Record];
                                newData[index].equipment_record =
                                  e.target.value;
                                setEditData({
                                  ...editData,
                                  Equipment_Record: newData,
                                });
                              }}
                              readOnly={
                                location.state?.stage !== 1 ||
