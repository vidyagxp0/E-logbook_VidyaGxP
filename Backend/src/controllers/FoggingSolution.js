    ...
    const newForm = await FoggingSolutionForm.create(
      {
        ...
        status: "Opened",
        stage: 1,
        nameOfFoggingSolution: nameOfFoggingSolution,
        quantityOfFoggingSolution: quantityOfFoggingSolution,
        quantityOfPurifiedWater: quantityOfPurifiedWater,
    ...
    if (Array.isArray(FormRecordsArray) && FormRecordsArray.length > 0) {
      ...
      formRecords.forEach((record, index) => {
        ...
        auditTrailEntries.push({
          ...
          previous_value: null,
          new_value: record.fogging_solution,
          changed_by: user.user_id,
          previous_status: "Not Applicable",
          new_status: "Opened",
          declaration: initiatorDeclaration,
...
exports.EditFoggingSolution = async (req, res) => {
  ...
  try {
    ...
    if (
      Array.isArray(FoggingSolutionRecords) &&
      FoggingSolutionRecords.length > 0
    ) {
      const existingRecords = await FoggingSolutionRecord.findAll({
        where: { form_id: form_id },
        raw: true,
      ...
      if (FoggingSolutionRecords.length > existingRecords.length) {
        for (
          let i = existingRecords.length;
          i < FoggingSolutionRecords.length;
          i++
        ) {
      ...
      await FoggingSolutionRecord.destroy({
        where: { form_id: form_id },
        transaction,
      });
...
exports.GetFoggingSolutionElog = async (req, res) => {
...
//get all the fogging solution elogs
exports.GetAllFoggingSolutionElog = async (req, res) => {
  FoggingSolutionForm.findAll({
    include: [
      {
        model: FoggingSolutionRecord,
      },
      {
        model: User,
        as: "reviewer9", // Use the consistent alias 'reviewer'
        attributes: ["user_id", "name"], // Specify which user attributes to fetch (optional)
      },
      {
        model: User,
        as: "approver9", // Use the consistent alias 'approver'
        attributes: ["user_id", "name"], // Specify which user attributes to fetch (optional)
      },
    ],
    order: [["form_id", "DESC"]],
...
exports.SendFSElogfromReviewToOpen = async (req, res) => {
...
exports.SendFSfromReviewToApproval = async (req, res) => {
...
exports.SendFSfromApprovalToOpen = async (req, res) => {
...
exports.ApproveFSElog = async (req, res) => {
...
exports.getAuditTrailForAnElog = async (req, res) => {
  try {
    ...
    const auditTrail = await FoggingSolutionAuditTrail.findAll({
